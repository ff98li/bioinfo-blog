...
### GEO metadata repository: <code>GEOmetadb</code>
...
The RNA-seq count matrix in the data file consists of **20460 unique Entrez gene ID's** along the rows and 86 samples (biological replicates) along the columns.
...
Note that the name for the column of gene ID's is blank.
...
a simple comparison between two sample groups: COVID-19 and HC.[@GSE152641] \
...
As the genes in this dataset were labeled with Entrez gene ID's,
...
and more meaningful than simply numerical ID's.
...
finish converting gene ID's in a later post. 
